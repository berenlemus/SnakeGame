# the original code is from https://github.com/ganeshkavhar/snake-game-in-python-by-ganesh-kavhar
# likewise instead of using turtle like the original code I am using tkinter
# I used some code from https://www.edureka.co/blog/snake-game-with-pygame/ which uses tkinter to create the UGI
# that allowed me to add a start, stop and quit button so the game was more user friendly
# I added the collison factor from another code from https://www.youtube.com/watch?v=SLkfoip1JQw which
# has the collison with the snake itself, and causes the user to lose
# I tried adding the option for users to change the color of the snake but it did not work

# Using tkinter for the graphics and randit to help with the movements of the snake
from tkinter import *
from random import randint

# constants that make the grid used for the snake's movement
GRADUATION = 40
PIXEL = 10
STEP = 2 * PIXEL
WD = PIXEL * GRADUATION
...
# color constants, setting the colors for the snake, background, and objects
BG_COLOR = 'white'
OB_COLOR = 'Pink'
SN_COLOR = 'Red'
...
# the first class is for creating the game canvas
class Master(Canvas):
    """here we create the game canvas, the snake, the obstacle, and keep track of the score"""
...
# added to set the default start for the game
    def start(self):
        """start snake game"""
        if self.running == 0:
...
# this is code I borrowed from my second source where it did keep track of the high score
...
# counting the amount of times the snake eats and object to track the score
    def increment(self):
...
class Shape:
    """This is a template to make objects and snake body parts"""
...
# the buttons for the start, stop and quit functions that were added to the original code
